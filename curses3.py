...
def controle(win, key, keys = [KEY_UP, KEY_DOWN, KEY_LEFT, KEY_RIGHT]):
	'''
	Controles de jeu
	paramètres :
...
def deplacement(win, score, key, snake, food):
	...
	if key == KEY_RIGHT:
		snake.insert(0, [0][0], snake[0][1]+1])
	...
	# la tête se déplace de 1 caractère vers le haut (ligne - 1)
		...
		snake.insert(0, [0][0]-1, snake[0][1]])
	...
	if snake[0][0] == win.getmaxyx()[1]:
		 snake[0][0] = 0

	if snake[0][1] == 0:
		 snake[0][1] == win.getmaxyx()[0]:

	if snake[0][0] == 0:
		 snake[0][0] == win.getmaxyx()[1]:

	if snake[0][1] == win.getmaxyx()[0]::
		 snake[0][1] = 0
